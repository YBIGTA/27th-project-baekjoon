...
def run_codes_and_compare(state: CounterexampleState) -> CounterexampleState:
    ...
    for i in range(100):
        ...
        input_gen_result = code_runner.run_code(test_case_generator, language)
        ...
        try:
            # 사용자 코드 실행
            user_result = code_runner.run_code_with_input(user_code, test_input, language)
            user_output = user_result.get("output", "").strip()
            user_outputs.append(user_output)
            
            # 올바른 해결책 실행
            correct_result = code_runner.run_code_with_input(correct_solution, test_input, language)
...
def execute_single_code(code: str, test_input: str, language: str) -> Dict[str, Any]:
    """단일 코드 실행"""
    code_runner = CodeRunnerClient()
    try:
        result = code_runner.run_code_with_input(code, test_input, language)
        return {
            "success": True,
